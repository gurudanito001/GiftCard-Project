...
import { getAllTransactions } from '@/lib/prisma/transactions';
...
const TransactionListItem = ({ id, userId, index, benefactor, beneficiary,  amount, type, category, status, createdAt }) => {
  ...
  return (
    <tr style={styles.tableRow}>
      ...
      <td className='py-3 text-start primary-text'> {type === "CREDIT" ? <CreditIcon /> : <DebitIcon /> }</td>
      ...
      <td className='py-3 text-start primary-text'><Link className='text-decoration-none small secondary-text w-100' href={`/allTransactions/${id}?userId=${userId}`}> View
